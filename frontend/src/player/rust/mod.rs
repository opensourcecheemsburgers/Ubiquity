...
use crate::player::rust::conversions::Sample;
...
impl Player {
    pub fn new(config: &Config, tx: Sender<PlayerMsg>) -> Self {
...
impl PlayerTrait for Player {
    ...
    fn seek(&mut self, secs: i64) -> std::result::Result<(), symphonia_core::errors::Error> {
        if secs.is_positive() {
            self.seek_fw();
            return Ok(());
        }
